...
Top level fields of the `package.json` file are converted into environment variables prefixed with `npm_package_`. For example, if your package.json contains this:
```bash
{
    "name": "foo",
    "version": "1.2.5"
...
See [`package.json`](/cli/v10/configuring-npm/package-json) for the full list of top level fields.
